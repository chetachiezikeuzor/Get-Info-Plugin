...
import GetInfoPlugin from "../plugin/main";
export class GetInfoSettingsTab extends PluginSettingTab {
	plugin: GetInfoPlugin;

	constructor(app: App, plugin: GetInfoPlugin) {
		super(app, plugin);
		this.plugin = plugin;
	}
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName("Set words per minute")
			.setDesc(
				"Set the average number of words used as a measurement for the speed of reading."
			)
			.addText((text) => {
				text.setValue(this.plugin.settings.wordsPerMinute).onChange(
					(newValue) => {
		...
		new Setting(containerEl)
			.setName("Set words per page")
			.setDesc(
				"Set the average number of words used as a measurement to calculate page count."
			)
			.addText((text) => {
				text.setValue(this.plugin.settings.wordsPerPage).onChange(
					(newValue) => {
		...
		new Setting(containerEl)
			...
			.setDesc("Toggle for whether counts should include footnotes.")
			.addToggle((text) => {
				text.setValue(this.plugin.settings.footnotesIncluded).onChange(
					(newValue) => {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Toggle for whether character counts should include spaces."
			)
			.addToggle((text) => {
				text.setValue(this.plugin.settings.spacesIncluded).onChange(
					(newValue) => {
		...
		new Setting(containerEl)
			...
			.setDesc("Toggle for whether counts should include comments.")
			.addToggle((text) => {
				text.setValue(this.plugin.settings.commentsIncluded).onChange(
					(newValue) => {
...
const kofiButton = (link: string): HTMLElement => {
	...
	a.setAttribute("href", link);
	a.addClass("buymeacoffee-chetachi-img");
	a.innerHTML = `<img src="https://raw.githubusercontent.com/chetachiezikeuzor/MySnippets-Plugin/master/assets//Users/Chetachi/Forest/.obsidian/plugins/Get-Info-Plugin/assets/Kofi_pixel_logo_with_text_dark.png" height="50">`;
	return a;
};
